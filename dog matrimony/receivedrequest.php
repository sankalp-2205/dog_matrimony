  ...
  <body>
   ...
   <source src="audio.mp3" type="audio/mpeg">
    ...
    <div class="navbar-collapse collapse" id="navbarcollapse">
        ...
        <li class="dropdown">
          <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" id="numberofnotifications" aria-haspopup="true" aria-expanded="false" onclick="notification()">Notification</a>
...
var open = false
 $('#sidebarCollapse').on('click', function () {
     ...
     $("#toggleclass").addClass("fa fa-times");
        ...
        // js code for received request ajax call-->
       $(function(){ 
        $("#receivedrequests").fadeOut();
           $("#spinner").show();
          $.ajax({
        url: "receivedrequestcode.php",
        type: "POST",
        success: function(data){
                    if(data)
                        {
                            $("#spinner").hide();
                             $("#receivedrequests").html(data);
                             $("#receivedrequests").slideDown();
                        }
        },
        error: function(){
            $("#spinner").hide();
            $("#receivedrequests").html("<div class = 'alert alert-danger'>Issue with ajax call.Plz try later </div>");
            $("#receivedrequests").slideDown();
        }
    });
    var interval;
    ...
    //  });
          ...
          // color of notification box after clicking
     });
     ...
     $(document).ready(function(){
        interval = setInterval(() => {
            ...
            console.log(<?php $_SESSION['notificationclicked']?>)
        },2000)
    })
     ...
     $(document).on("click", "#accept_request", function(event){
    event.preventDefault();
    var buttonname = $(this).attr('name'); 
    var id = extractid(buttonname);
    var datatopost = {'sentfrom' : id.sentfrom,
    'sentto' : id.sentto};
    ...
    $.ajax({
                    ...
                    if(data == "success")
                        {
                            ...
                            $('#accept_request[name="'+buttonname+'"]').attr("disabled",true);
                            $('#accept_request[name="'+buttonname+'"]').css("background-color","#e3e3e3");
                            $('#accept_request[name="'+buttonname+'"]').css("color","green");
                            $('#accept_request[name="'+buttonname+'"]').html("Confirmed");
                        }
                        if(data == "reload")
                        {
                            location.reload(true);
        ...
        error: function(){
            $("#test").html("<div class = 'alert alert-danger'>Issue with ajax call.Plz try later </div>");
        }
    });
});
$(document).on("click", "#decline_request", function(event){
    event.preventDefault();
    var confirmation = confirm("Are you sure you want to decline request.Once Declined it cannot be undone?");
    var buttonname = $(this).attr('name'); 
    var id = extractid(buttonname);
    var datatopost = {'sentfrom' : id.sentfrom,
    'sentto' : id.sentto};
    ...
    else
   {
        ...
        error: function(){
            $("#test2").html("<div class = 'alert alert-danger'>Issue with ajax call.Plz try later </div>");
        }
    });
}
});   
