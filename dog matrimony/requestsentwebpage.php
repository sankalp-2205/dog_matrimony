            ...
            <div class="valid-feedback mb3" id = "weighterror" style = "color: red"></div>
             ...
             <label for="breed">Gender:</label><span class="required">*</span>
                <select id="gender" class="form-control" name = "gender">
                   <option selected>Select gender</option><span class="required">*</span>
                    <option>Male</option>
...
var open = false
 $('#sidebarCollapse').on('click', function () {
     ...
     $("#toggleclass").addClass("fa fa-times");
       ...
       $(function(){  
        $("#sentrequests").fadeOut();
           $("#spinner").show();
          $.ajax({
        url: "requestsentwebpagecode.php",
        type: "POST",
        success: function(data){
                    if(data)
                        {
                            $("#spinner").hide();
                             $("#sentrequests").html(data);
                             $("#sentrequests").slideDown();
                        }
        },
        error: function(){
            $("#spinner").hide();
            $("#sentrequests").html("<div class = 'alert alert-danger'>Issue with ajax call.Plz try later </div>");
            $("#sentrequests").slideDown();
        }
    });
     });
    ...
    else
   {
       console.log(buttonname);
        ...
        success: function(data){
                    ...
                    if(data == "reload")
                    {
                        alert("The request has already been declined by the other user");
                        location.reload(true);
                    }
                        else
                        {
        ...
        error: function(){
            $("#cancelmessage").html("<div class = 'alert alert-danger'>Issue with ajax call.Plz try later </div>");
        }
    });
}
});  
