  ...
  <audio class="audios" style = "display: none;" id="audio" controls preload="none"> 
   <source src="audio.mp3" type="audio/mpeg">
    ...
    <div class="navbar-collapse collapse" id="navbarcollapse">
        ...
        <li><a href = "http://websh.offyoucode.co.uk/alternate/index.php">Log out</a></li>
        </ul>
    </div>
    </div>  
    </nav>
...
var datatopost = {};
var buttonname = GetURLParameter('data');      
...
const dogid = arr[0];
       $(function(){  
        ...
        error: function(){
            $("#spinner").hide();
            $("#matches").html("<div class = 'alert alert-danger'>Issue with ajax call.Plz try later </div>");
            $("#matches").slideDown();
        }
   });
    });
    ...
    console.log($(this).attr('name'));
      var sentto = $(this).attr('name');
      var sentfrom = buttonname;
       arr = sentto.split('[');
        arr = arr[1].split(']');
        var sentto2 = arr[0];
        arr = sentfrom.split('[');
        arr = arr[1].split(']');
        var sentfrom2 = arr[0];
        var datatopost = {"sentfrom": sentfrom2, "sentto": sentto2};
      ...
      $.ajax({
                    ...
                    if(data == 'doesnotexist')
                        {
                            ...
                            alert("This dog does not exist. Plz refresh the page")
                            location.reload(true);
                        }
                        if(data == "success")
                        {
                            $(this).html("Sent");
                              window.location.href="http://websh.offyoucode.co.uk/dog%20matrimony/sentrequest.php?sentfrom="+encodeURIComponent(buttonname)+"&sentto="+encodeURIComponent(sentto);
                        }
                        else
                        {
                            $(this).attr("disabled",false);
        ...
        error: function(){
            alert("Issue with the ajax call. Plz try again later");
        }
   });
