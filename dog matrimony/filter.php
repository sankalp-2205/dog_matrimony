...
<input class='form-check-input' name='filter' type='radio' id='all' value='all' onClick = 'update()'>
<label class='form-check-label'>All dates</label>
</div>
</div>
<div class = 'col-md-3'>
<div class='form-check mb-4'>
<input class='form-check-input' name='filter' type='radio' id='select' value='select' onClick = 'update()' checke>
<label class='form-check-label'>Filter By Date</label>
</div>
</div>
<div class = 'col-md-3'>
<div class='form-group'>
<input name = 'filterdate' id = 'filterdate' type='text' class='form-control' value = '$date'>
</div>
</div>
</div>
</div>";
$sql = "SELECT * FROM appointments WHERE organisation_name = '$organisation_name' AND appointment_date = '$date'";
if($result = $link ->query($sql))
{
    while($rows = $result->fetch_array(MYSQLI_ASSOC))
    {  
